/* === e-Rapor Cerdas - Dashboard Script (Final V3 + Fix Input/Simpan) === */
...
document.addEventListener("DOMContentLoaded", () => {
    console.log("DOM siap."); // Log Sederhana 1
    try {
        // 1. OTENTIKASI
        ...
        if (!user.name || !user.spreadsheetId || !user.username) { console.error("Auth Check >> Gagal:", user); alert("Sesi berakhir. Login ulang."); window.location.href = 'index.html'; return; }
        console.log("Auth Check >> Berhasil:", user.spreadsheetId); // Log ID sukses
        if (welcomeMsgEl) welcomeMsgEl.innerText = `Selamat Datang, ${user.name}!`; else console.error("#welcome-message missing.");

        // 2. LOGIKA LOGOUT
        const logoutBtn = document.getElementById('logout-button');
        if (logoutBtn) { logoutBtn.addEventListener('click', (e) => { e.preventDefault(); if (confirm("Yakin logout?")) { localStorage.clear(); window.location.href = 'index.html'; } }); } else console.error("#logout-button missing.");
        ...
        if (nilaiAkhirInput) {
            nilaiAkhirInput.addEventListener('input', validateAndToggleButton); // Panggil validasi saat nilai diketik
        ...
        if (toggleSiswaListBtn && siswaTableContainer) { toggleSiswaListBtn.addEventListener('click', (e) => { e.preventDefault(); const isExpanded = siswaTableContainer.classList.toggle('is-expanded'); toggleSiswaListBtn.innerText = isExpanded ? 'Sembunyikan' : 'Tampilkan Semua Siswa'; }); } else console.warn("#toggle-siswa-list or .data-table-container missing.");

        // 9. LOGIKA TOMBOL SIMPAN NILAI
        if (simpanNilaiBtn && formInputNilai) {
             simpanNilaiBtn.disabled = true; // Nonaktifkan di awal
             simpanNilaiBtn.addEventListener('click', (e) => { e.preventDefault(); handleSimpanNilai(); }); // Listener tetap ada
        ...
        console.log("Inisialisasi DOM selesai."); // Log Sederhana 2
...
function generateFinalDescription() {
    ...
    if (!isMulokActive) {
        ...
        const namaSiswa = siswaSelectedIndex > 0 && selectSiswa.options.length > siswaSelectedIndex ? selectSiswa.options[siswaSelectedIndex].text : "";
        ...
        const prefixNama = namaSiswa ? `Ananda ${namaSiswa}` : "Ananda"; // Selalu "Ananda"

        if (deskripsiTercapaiList.length > 0) { finalDescription += prefixNama + pembukaTercapai + deskripsiTercapaiList.join(', '); }
        if (deskripsiBimbinganList.length > 0) { finalDescription += (finalDescription !== "" ? ", namun" : prefixNama + " ") + pembukaBimbingan + deskripsiBimbinganList.join(', '); }
    ...
    if (!isAnyCpSelected && !isMulokActive) {
        finalDescriptionInput.placeholder = "Pilih status capaian..."; finalDescriptionInput.readOnly = true; if (editDeskripsiBtn) editDeskripsiBtn.disabled = true;
    } else {
        finalDescriptionInput.placeholder = isMulokActive ? "Input Mulok..." : "Deskripsi rapor..."; finalDescriptionInput.readOnly = false; if (editDeskripsiBtn) editDeskripsiBtn.disabled = isMulokActive;
    }
    // Panggil validasi setelah deskripsi berubah
    validateAndToggleButton();
}
...
function resetMulok() {
    ...
    if (editDeskripsiBtn) editDeskripsiBtn.style.display = 'block'; // Tampilkan lagi tombol edit
}
...
function validateAndToggleButton() {
    ...
    // console.log(`Validasi: Tombol Simpan disabled = ${simpanNilaiBtn.disabled}`); // Hapus log ini jika sudah OK
}
